...
MSPISlaveClass::MSPISlaveClass(spi_inst_t *spi, pin_size_t rx, pin_size_t cs, pin_size_t sck,
                               pin_size_t tx) {
  _spi      = spi;
  _running  = false;
  _initted  = false;
...
inline spi_cpol_t MSPISlaveClass::cpol(SPISettings _spis) {
...
inline spi_cpha_t MSPISlaveClass::cpha(SPISettings _spis) {
  switch (_spis.getDataMode()) {
  case SPI_MODE0:
    return SPI_CPHA_0;
  case SPI_MODE1:
...
bool MSPISlaveClass::setRX(pin_size_t pin) {
  constexpr uint32_t valid[2] = {
  ...
  if (_running) {
    ...
    panic("FATAL: Attempting to set SPI%s.RX to illegal pin %d", spi_get_index(_spi) ? "1" : "",
          pin);
  }
  return false;
}

bool MSPISlaveClass::setCS(pin_size_t pin) {
  constexpr uint32_t valid[2] = {
  ...
  if (_running) {
    ...
    panic("FATAL: Attempting to set SPI%s.CS to illegal pin %d", spi_get_index(_spi) ? "1" : "",
          pin);
  }
  return false;
}

bool MSPISlaveClass::setSCK(pin_size_t pin) {
  constexpr uint32_t valid[2] = {
  ...
  if (_running) {
    ...
    panic("FATAL: Attempting to set SPI%s.SCK to illegal pin %d", spi_get_index(_spi) ? "1" : "",
          pin);
  }
  return false;
}

bool MSPISlaveClass::setTX(pin_size_t pin) {
  constexpr uint32_t valid[2] = {
  ...
  if (_running) {
    ...
    panic("FATAL: Attempting to set SPI%s.TX to illegal pin %d", spi_get_index(_spi) ? "1" : "",
          pin);
  }
  return false;
}

void MSPISlaveClass::_handleIRQ() {
...
void MSPISlaveClass::_irq0() {
  SPISlave._handleIRQ();
}

void MSPISlaveClass::_irq1() {
  SPISlave1._handleIRQ();
}

void MSPISlaveClass::setData(const uint8_t *data, size_t len) {
  _dataOut  = data;
  _dataLeft = len;
  if (_initted) {
    // Enable the TX FIFO IRQ
    spi_get_hw(_spi)->imsc = 2 | 4 | 8;
  }
}

void MSPISlaveClass::begin(SPISettings spis) {
...
MSPISlaveClass SPISlave(__SPI0_DEVICE, PIN_SPI0_MISO, PIN_SPI0_SS, PIN_SPI0_SCK, PIN_SPI0_MOSI);
MSPISlaveClass SPISlave1(__SPI1_DEVICE, PIN_SPI1_MISO, PIN_SPI1_SS, PIN_SPI1_SCK, PIN_SPI1_MOSI);
