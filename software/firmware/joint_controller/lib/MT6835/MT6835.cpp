/**
 * @file MT6835.cpp
 * @brief Implementation of MT6835 21-bit magnetic encoder driver
 *
 * See MT6835.h for class documentation and feature details.
 */
...
MT6835::MT6835(SPISettings settings, int nCS)
    : settings(settings), nCS(nCS) {
  // Constructor - SPI settings and CS pin stored
}

MT6835::~MT6835() {
  // Destructor - cleanup handled by Arduino
}
...
float MT6835::getCurrentAngle() {
  ...
  if (checkcrc) {
    if (lastcrc != calcCrc(rawangle, laststatus)) {
      ...
      return -1; // return -1 to signal CRC error - the current angle has to be non-negative
                 // otherwise
    }
  }
...
/**
 * @brief Calculate CRC8 of angle and status data
 *
 * Polynomial: x^8 + x^2 + x + 1 = 0x07 (0xE0 reflected)
 * Initial value: 0x00
 * No final XOR
 *
 * @param angle 21-bit angle value
...
uint16_t MT6835::getABZResolution() {
  ...
  MT6835ABZRes lo = {.reg = readRegister(MT6835_REG_ABZ_RES2)};
...
uint16_t MT6835::getZeroPosition() {
  ...
  MT6835Options0 lo = {.reg = readRegister(MT6835_REG_ZERO2)};
...
MT6835Options1 MT6835::getOptions1() {
  MT6835Options1 result = {.reg = readRegister(MT6835_REG_OPTS1)};
  return result;
};
...
MT6835Options2 MT6835::getOptions2() {
  MT6835Options2 result = {.reg = readRegister(MT6835_REG_OPTS2)};
  return result;
};
...
MT6835Options3 MT6835::getOptions3() {
  MT6835Options3 result = {.reg = readRegister(MT6835_REG_OPTS3)};
  return result;
};
...
MT6835Options4 MT6835::getOptions4() {
  MT6835Options4 result = {.reg = readRegister(MT6835_REG_OPTS4)};
  return result;
};
